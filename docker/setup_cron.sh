...
: "${EBIRD_REGION="MAJOR_REGION:world"}" # default to all sightings
set -ef
echo "using region: $EBIRD_REGION"
echo "using refresh interval: $REFRESH_INTERVAL"
echo "$REFRESH_INTERVAL /usr/local/bin/python3 -m bird_brained --location $EBIRD_REGION --headless > /proc/1/fd/1 2>&1" >> /etc/cron.d/brid_brained
